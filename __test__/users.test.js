...
describe("GET User By Id - For Admin Account", () => {
  describe("Success fetch user details", () => {
      ...
      res = await request(app).get("/users/1").set("access_token", access_token);;
			...
			expect(res.body.userById).toHaveProperty("phoneNumber", expect.any(String));
			...
			expect(res.body.userById).toHaveProperty("isPremium", expect.any(Boolean));
	...
	describe("Failed fetch user details", () => {
      ...
      res = await request(app).get("/users/100").set("access_token", access_token);
...
describe("PUT Admin - Update User", () => {
	describe("Success change user data from Admin", () => {
      ...
      let res = await request(app).post("/login").send(body);
			...
			res = await request(app).put("/users/1").send(passwordChange).set("access_token", access_token);
  ...
  describe("Failed change user password because less than 5 word from Admin", () => {
      ...
      let res = await request(app).post("/login").send(body);
			...
			expect(res.status).toBe(400);
			expect(res.body).toBeInstanceOf(Object);
			expect(res.body).toHaveProperty("msg", "Minimum password length must be 5 letter");
		});
	});
  ...
  describe("Failed change user full name from Admin", () => {
      ...
      let res = await request(app).post("/login").send(body);
			...
			let nameChange = { fullName: "", phoneNumber: "08111945586", email: "bobby@gmail.com", password: "12345" };
			res = await request(app).put("/users/1").send(nameChange).set("access_token", access_token);
  ...
  describe("Failed change user phone number from Admin", () => {
      ...
      let res = await request(app).post("/login").send(body);
			...
			let numberChange = { fullName: "Bobby", phoneNumber: "", email: "bobby@gmail.com", password: "12345" };
			res = await request(app).put("/users/1").send(numberChange).set("access_token", access_token);
  ...
  describe("Failed change user email from Admin", () => {
      ...
      let res = await request(app).post("/login").send(body);
			...
			let emailChange = { fullName: "Bobby", phoneNumber: "08111", email: "", password: "12345" };
			res = await request(app).put("/users/1").send(emailChange).set("access_token", access_token);
  ...
  describe("Failed change user email because duplication from Admin", () => {
      ...
      let res = await request(app).post("/login").send(body);
			...
			let emailChange = { fullName: "Bobby", phoneNumber: "08111", email: "bobby25@gmail.com", password: "12345" };
			res = await request(app).put("/users/1").send(emailChange).set("access_token", access_token);
...
describe("PATCH Admin - Update User Status To Premium", () => {
  describe("Success change user premium status from Admin", () => {
      ...
      let res = await request(app).post("/login").send(body);
			...
			res = await request(app).patch("/users/activatePremium/1").set("access_token", access_token);

			expect(res.status).toBe(200);
			expect(res.body).toBeInstanceOf(Object);
			expect(res.body).toHaveProperty("message", "User status has been updated to premium");
		});
	});
...
describe("PATCH Admin - Update User Status From Premium To Not", () => {
  describe("Success change user premium status from Admin", () => {
      ...
      let res = await request(app).post("/login").send(body);
			...
			res = await request(app).patch("/users/deactivatePremium/1").set("access_token", access_token);

			expect(res.status).toBe(200);
			expect(res.body).toBeInstanceOf(Object);
			expect(res.body).toHaveProperty("message", "User status no longer premium");
		});
	});
